 ...
 * out as appropriate.
 */
public class SoftKeyboard extends InputMethodService implements KeyboardView.OnKeyboardActionListener, OnClickListener {
	...
	public void onCreate() {
		...
		// if (mDictionary != null)
		// System.out.println(" Dictionary " + mDictionary);

		mDictionary.setDictionaryUpdateListener(new Dictionary.DictionaryUpdateListener() {
			@Override
	...
	private void updateShiftKeyState(EditorInfo attr) {
		// if (attr != null && mInputView != null && mQwertyKeyboard ==
	...
	public void onKey(int primaryCode, int[] keyCodes) {
		...
		if (primaryCode == 32) {
			...
			// View popupView = layoutInflater.inflate(R.layout.laypp, null);
			//
			...
			// WindowManager.LayoutParams(displayWidth - 10,
			// displayHeight - 125);
	...
	private void updateCandidates() {
		...
		List<String> listData = mDictionary.readAll();
		ArrayList<String> list = new ArrayList<String>();
		if (!mCompletionOn) {
			mPick = false;
			if (mComposing.length() > 0) {
				list.add(mComposing.toString());
				String string = mComposing.toString().toLowerCase();
				if (!mDictionary.match(string)) {
					mFlag = true;

				} else
					mFlag = false;
				for (int i = 0; i < listData.size(); i++) {
					String listString = listData.get(i).toLowerCase();
					if (listString.startsWith(string)) {
						if (i > CandidateView.MAX_SUGGESTIONS)
							break;
						list.add(listString);
						boolean isLen = listString.length() == string.length();
						if (isLen && listString.length() >= 3 && listString.equalsIgnoreCase(string))
							list.remove(0);
					} else {
					}
				}
	...
	public void pickSuggestionManually(int index) {
		...
		if (mCompletionOn && mCompletions != null && index >= 0 && index < mCompletions.length) {
			...
			if (index == 0 && mFlag && mComposing.length() >= 3) {
				mCandidateList.clear();

				mSaveWord = mComposing.toString();
				mComposing = new StringBuilder(mComposing + " ");
				mCandidateList.add(mSaveWord + " | Touch again to save.");
				mSave = true;
				mIndex = index;
				mFlag = true;
				index = -1;
				...
				String pickString = mCandidateList.get(index) + " ";
				mComposing = new StringBuilder(pickString);
				commitTyped(getCurrentInputConnection());
			}
	...
	private void savePicKSuggestionManually() {
		if (mPick && mIndex == 0 && mFlag) {
			mPick = false;
			mFlag = false;
			mIndex = -1;
			if (mSaveWord != null)
				Toast.makeText(getApplicationContext(), "Save: " + mSaveWord, Toast.LENGTH_SHORT).show();
			mDictionary.writeFile(mSaveWord);
			setSuggestions(null, false, false);
			Toast.makeText(getApplicationContext(), "candidate clear", Toast.LENGTH_SHORT).show();
		}
	}
	...
	public void ShowKeyboard(View pView) {
		...
		if (pView != null) {
			...
			imm.showSoftInput(pView, InputMethodManager.HIDE_IMPLICIT_ONLY);
			customToast("not null");
	...
	public void HideKeyboard(View pView) {
		...
		inputMethodManager.hideSoftInputFromInputMethod(pView.getWindowToken(), 0);
	}
